...
exports.handler = async (event) => {
  ...
  try {
    const { weight, height, goalType } = JSON.parse(event.body);
    ...
    if (!weight || weight <= 0 || weight > 500) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: '请输入有效的体重 (1-500 kg)' })
      };
    }
    ...
    if (!height || height <= 0 || height > 300) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: '请输入有效的身高 (1-300 cm)' })
      };
    }
    ...
    if (!goalType) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: '请选择目标类型' })
      };
    }
